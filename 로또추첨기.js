// var 후보군 = Array(45);    // 많은 배열 할때는 Array함수 사용, Arra(y함수통해 빈 배열 생성. 대부분의 경우에는 배열 만들때 []이용
// var 필 = 후보군.fill();
// console.log(필);
//
...
var 셔플 = [];
while (후보군.length > 0) {
  var 이동값 = 후보군.splice(Math.floor(Math.random() * 후보군 ))
  셔플.push(이동값);
}
console.log(셔플);
...
setTimeout(function 비동기콜백함수() {
  var 칸 = doucment.getElementByClassName('보너스') [0];
  공색칠하기(보너스, 칸);
}, 7000);